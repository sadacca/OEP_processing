...
[filename,filepath]=uigetfile(...
    '*.cont*','oep continuous files as generated by OpenEphys','MultiSelect','on');

%% load the data
%get there
cd(filepath)

if iscell(filename) % if you've got multiple channels
    
    %open the first channel
    
    [first_channel, timestamps, info_continuous] = load_open_ephys_data_faster(filename{1});
    ...
    [incoming_data(ii,:), timestamps, info_continuous] = load_open_ephys_data_faster(filename{ii});
    ...
    disp(['files ',filename{1},' to ',filename{end},' converted'])
    
    
else % else if you only have one channel or file
    
    % open the one file
    [incoming_data] = load_open_ephys_data_faster(filename{1});
